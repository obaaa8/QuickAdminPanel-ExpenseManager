    ...
    <div class="card-body">
        <form action="{{ route("admin.customers.store") }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="form-group {{ $errors->has('name') ? 'has-error' : '' }}">
                ...
                <input type="text" id="name" name="name" class="form-control" value="{{ old('name', isset($customer) ? $customer->name : '') }}" required>
                @if($errors->has('name'))
            ...
            <div class="form-group {{ $errors->has('password') ? 'has-error' : '' }}">
                ...
                @endif
                <p class="helper-block">
                    {{ trans('cruds.customer.fields.password_helper') }}
                </p>
            </div>
            {{-- <div class="form-group {{ $errors->has('roles') ? 'has-error' : '' }}">
                <label for="roles">{{ trans('cruds.customer.fields.roles') }}*
                    <span class="btn btn-info btn-xs select-all">{{ trans('global.select_all') }}</span>
                    <span class="btn btn-info btn-xs deselect-all">{{ trans('global.deselect_all') }}</span></label>
                <select name="roles[]" id="roles" class="form-control select2" multiple="multiple" required>
                    @foreach($roles as $id => $roles)
                        <option value="{{ $id }}" {{ (in_array($id, old('roles', [])) || isset($customer) && $customer->roles->contains($id)) ? 'selected' : '' }}>{{ $roles }}</option>
                    @endforeach
                </select>
                @if($errors->has('roles'))
                    <em class="invalid-feedback">
                ...
                @endif
                <p class="helper-block">
                    {{ trans('cruds.customer.fields.roles_helper') }}
                </p>
            </div> --}}
            <div>
